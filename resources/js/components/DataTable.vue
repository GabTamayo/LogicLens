...
const handleFilterInput = debounce((column: string, value: string) => {
    const formattedValue = column === 'student_name' ? value.replace(/\b\w/g, (c) => c.toUpperCase()) : value;
...
<template>
    <div class="space-y-4">
        <div v-if="filterConfigs.length > 0" class="flex flex-col gap-2 py-2 sm:flex-row sm:items-center sm:gap-4">
            <Input
            ...
            <DropdownMenu v-else>
                <DropdownMenuTrigger as-child>
                    <Button variant="outline" class="mr-auto">
                        ...
                        <EllipsisVertical class="ml-2 h-4 w-4 sm:ml-0" />
                    </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" class="w-61">
                    <DropdownMenuCheckboxItem
        ...
        <div class="rounded-md border">
            <Table>
                ...
                <TableBody>
                            ...
                            <TableRow v-if="row.getIsExpanded()">
                                <TableCell :colspan="row.getAllCells().length">
                                    <div class="max-h-200 overflow-auto">
                                        <pre
                                            class="line-numbers"
                                        ><code :class="`language-${getLanguageFromExtension(row.original.language)}`">{{ row.original.code_content }}
                                            </code>
                                        </pre>
                                    </div>
                                </TableCell>
                    ...
                    <template v-else>
                        <TableRow>
                            <TableCell :colspan="columns.length" class="h-24 text-center text-muted-foreground"> No results. </TableCell>
                        </TableRow>
                    </template>
                </TableBody>
            </Table>
