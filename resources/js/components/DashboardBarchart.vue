...
<template>
    <Card>
                ...
                <Select v-model="selectedFilter">
                    ...
                    <SelectContent>
                        ...
                        <SelectGroup v-if="groupedActivities['java']?.length > 0">
                            ...
                            <SelectItem value="java"> All <span class="text-xs text-muted-foreground">(Java)</span> </SelectItem>
                            <SelectItem v-for="item in groupedActivities['java']" :key="item.activity_id" :value="item.activity_id">
                                {{ item.activity_title }}
                            </SelectItem>
                            <SelectSeparator />
                        </SelectGroup>
                        ...
                        <SelectGroup v-if="groupedActivities['python']?.length > 0">
                            ...
                            <SelectItem value="python"> All <span class="text-xs text-muted-foreground">(Python)</span> </SelectItem>
                            <SelectItem v-for="item in groupedActivities['python']" :key="item.activity_id" :value="item.activity_id">
                                {{ item.activity_title }}
                            </SelectItem>
                        </SelectGroup>
                    </SelectContent>
        ...
        <CardContent class="h-120 px-2 pt-4 pb-4 sm:px-6 sm:pt-6">
            ...
            <ChartContainer v-else :config="chartConfig">
                <VisXYContainer :data="chartData" :margin="{ left: 0 }" :y-domain="[0, 100]">
                    <VisGroupedBar :x="(d: Data, i: number) => i" :y="(d: Data) => d.score" :color="chartConfig.score.color" :rounded-corners="25" />
                    <VisAxis
                        type="x"
                        :x="(d: Data, i: number) => i"
                        :tick-line="false"
                        ...
                        "
                    />
                    <VisAxis type="y" :num-ticks="8" :tick-line="false" :domain-line="false" :tick-format="(d: number) => `${d}%`" />
                    <ChartTooltip />
                    <ChartCrosshair
                        :template="
