...
const openActivities = computed(() =>
    props.activities.filter((activity) => activity.is_open)
);
    ...
    <ScrollArea class="h-full">
        <Accordion type="multiple" :default-value="['open']" class="w-full p-4">
            <AccordionItem value="open">
                ...
                <AccordionContent>
                    <div class="flex flex-col gap-1 pt-2">
                        <div v-for="activity in openActivities" :key="activity.id" @click="emit('select', activity)"
                            :class="[
                            ...
                            <div class="flex items-center gap-2">
                                <Badge variant="outline" :class="[
                                    ...
                                    <img v-if="getLanguageConfig(activity.activity_language).logo"
                                        ...
                                        :alt="activity.activity_language" class="h-3 w-3 object-contain" />
                                    <span>{{ activity.activity_language }}</span>
                                </Badge>
                            </div>
                        </div>
                        <div v-if="openActivities.length === 0" class="py-8 text-center text-sm text-muted-foreground">
                            No open activities
                        </div>
                    </div>
                </AccordionContent>
            ...
            <AccordionItem value="closed">
                ...
                <AccordionContent>
                    <div class="flex flex-col gap-1 pt-2">
                        <div v-for="activity in closedActivities" :key="activity.id" @click="emit('select', activity)"
                            :class="[
                                'group flex cursor-pointer flex-col gap-2 rounded-lg border p-3 transition-all hover:bg-accent',
                                selectedActivityId === activity.id
                                    ? 'border-primary bg-accent'
                        ...
                        <div v-if="closedActivities.length === 0"
